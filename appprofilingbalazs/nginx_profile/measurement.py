...
with open('config.yaml') as config_file:
    ...
    for i in range(1,max_pod+1):
        ...
        while loop < max_benchmark_iteration:
            ...
            try:
               res = json.loads((requests.get(url="http://{0}:8080/fortio/?labels=Fortio&url=http://172.16.0.7:30044&qps={2}&t={3}s&n=&c=8&p=50, 75, 90, 99, 99.9&r=0.0001&H=Host: nodejsapp&H=User-Agent: fortio.org/fortio-1.3.2-pre&H=&H=&H=&runner=http&resolve=&grpc-ping-delay=0&json=on&save=on&load=Start&timeout=100ms".format(fortio_ip,nodejs_ip,qps,benchmark_time))).text)
        ...
        with open('{0}.json'.format('nodejs_'+datetime.datetime.strftime(datetime.datetime.now(),'%y_%m_%d_%H_%M_%S')+'_'+str(pod)),'w') as file_result:
