...
with open('config.yaml') as config_file:
   ...
   dirname = '{0}'.format('nodejs_'+datetime.datetime.strftime(datetime.datetime.now(),'%y_%m_%d_%H_%M_%S')+'_'+str(measurement_id))

   if not os.path.exists(join('logs',dirname)):
      os.makedirs(join('logs',dirname))


   config = yaml.safe_load(config_file)
   max_cpu = str(config['measurement']['max_cpu'])
   cpus = config['measurement']['cpus']
   pods = config['measurement']['pods']
   max_pod = len(cpus)
   if(len(pods) != len(cpus)):
      raise RuntimeError('Len of pods and cpus in config are not equal')
   ...
   print('--------------------------------------------------\n')
   print('-------------------- Start Deploy ----------------\n')
   subprocess.call(['./deploy.sh',config['deploy']['sut_worker'],config['deploy']['benchmark_worker'],'1',cpus[0],'live_nodejs_app_{0}.yaml'.format(measurement_id),str(measurement_id)])
   #raise Error('ASD')
   subprocess.call(['./wait_to_be_running.sh',str(2),str(measurement_id)])
   fortio_ip ='' 
   nodejs_ip =''
   with open("live_config_{0}.yaml".format(measurement_id)) as live_config:
   ...
   for i in range(0,len(pods)):
      ...
      subprocess.call(['./update_nodejs_app.sh',str(pod),cpu,str(measurement_id)])
      subprocess.call(['./wait_to_be_running.sh',str(pod+1),str(measurement_id)])
      #time.sleep(60)
      lost_requests_percentage = 0
      qps = start_qps
      loop=0
      failed_meas = 0
      while loop < max_benchmark_iteration and failed_meas<=2:
         loop+=1
         print('        ------- Benchmark - qps: {0} ------'.format(qps))
         # TODO call benchmark script
         start_time = time.time() 
         try:
            res = json.loads((requests.get(url="http://{0}:8080/fortio/?labels=Fortio&url=http://172.16.0.7:30044&qps={2}&t={3}s&n=&c=10&p=50, 75, 90, 99, 99.9&r=0.0001&H=Host: nodejsapp-{4}&H=User-Agent: fortio.org/fortio-1.3.2-pre&H=&H=&H=&runner=http&resolve=&grpc-ping-delay=0&json=on&save=on&load=Start&timeout=1s".format(fortio_ip,nodejs_ip,qps,benchmark_time,measurement_id))).text)
            ...
            print(start_time)
            print(end_time)
            failed_meas = failed_meas+1 if lost_requests_percentage > max_request_lost else 0
            try:
               # TODO collect CPU, MEM, Network info from prometheus
               cpu_usage = json.loads(requests.get(url="http://10.106.36.184:9090/api/v1/query_range?query=sum(rate(container_cpu_usage_seconds_total{{container_name=\"nodejs-app-{4}\"}}[{3}s]))&start={0}&end={1}&step={2}".format(start_time,end_time,1,10,str(measurement_id))).text)
               ...
               measurement['values'].append({'qps':qps,'avg_cpu':cpu_avg,'req_lost':lost_requests_percentage,'avg_mem':'N/A','cpu_values':cpu_values,'mem_values':[]})
               # TODO calculate CPU usage, lost_requests_percentage
         qps = qps if qps != 1 else 0
         qps += step
      ...
      with open(join('logs',dirname,'{0}.json'.format('nodejs_'+datetime.datetime.strftime(datetime.datetime.now(),'%y_%m_%d_%H_%M_%S')+'_'+str(pod))),'w') as file_result:
   ...
   subprocess.call(['./delete.sh',str(measurement_id)])
