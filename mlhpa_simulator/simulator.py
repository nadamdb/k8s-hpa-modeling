...
class Connector(object):
    ...
    def __init__(self, model, loadfile):
        self.service = None
        self.model = model
        self.loadfilename = ntpath.basename(loadfile)
        self.loadfile = loadfile
        with open(loadfile, "r") as f:
            log = json.load(f)
        if log is None:
            raise FileNotFoundError
        self.send_times = log["data"]["load_send_time"]
        self.send_times_df = pd.DataFrame(log["data"]["load_send_time"], columns=["send_time"])
        self.current_time = 0
    ...
    def push_cpu_metric(self):
        raw_data = '[]'
        while raw_data == '[]' or (type(raw_data) == list and len(raw_data) == 0):
            query = 'sum(rate(container_cpu_usage_seconds_total{pod_name=~"'+DEPLOYMENT_NAME+'.*",container_name!="POD"}[1m]))'
            raw_data = self.query_prometheus(query)
...
def main(model, loadfile):
    connector = Connector(model, loadfile)
    connector.run()


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Scaling predictor')
    parser.add_argument('--model', '-m', type=str, help='Set the machine learning model. It can be: lstm, ar')
    parser.add_argument('--load', '-l', type=str, help='Load file')
    args = parser.parse_args()
    main(args.model, args.load)
